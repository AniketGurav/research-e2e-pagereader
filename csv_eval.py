...
def _get_detections(dataset, retinanet, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            if indices.shape[0] > 0:
                ...
                transcriptions = transcriptions[indices[scores_sort]]
                transcriptions=np.argmax(transcriptions,axis=-1)
                image_detections = np.concatenate([image_boxes, np.expand_dims(image_scores, axis=1), np.expand_dims(image_labels, axis=1)], axis=1)

                # copy detections to all_detections
                all_transcriptions[index]={}
                for label in range(dataset.num_classes()):
                    all_detections[index][label] = image_detections[image_detections[:, -1] == label, :-1]
                    all_transcriptions[index][label]=transcriptions[image_detections[:, -1] == label, :-1]
    ...
    return all_detections,all_transcriptions
...
def _get_annotations(generator):
    ...
    for i in range(len(generator)):
        ...
        for label in range(generator.num_classes()):
            ...
            all_transcriptions[i][label]=annotations[annotations[:, 4] == label, 5:].copy()
